function [mask, skull] = process_brain(image, neeta, neetab)
...
thinthinoutline=imbinarize(im2uint8(bwmorph(blobnew,'thicken',1)),'adaptive','sensitivity', 0.0);
...
grf=adapthisteq(grf,'NumTiles',[round(imx/70) round(imy/70)],'Distribution','uniform', 'NBins',256,'ClipLimit',0.004);    %was 256 & 0.005
grf=imadjust(grf,[],[]);
...
finsil = bwmorph(finsil,'thicken',4);
...
brain=adapthisteq(brain,'NumTiles',[round(x/15) round(y/15)],'Distribution','rayleigh', 'NBins',18,'ClipLimit',0.00,'Alpha', 0.9);  %nbins 11 alpha 0.9   %wnbins 18
...
intensities = find(histo>0 & histo<round(0.05*max(histo)));
...
se11 = strel('disk',round(min(msx,msy)*0.018));
